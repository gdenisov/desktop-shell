...
`references` and `scope` describe the app for review, test, freshness, and publish passes; `forward_port.py` copies neither onto the registry row, so a bad reference can never stop the app from starting. The rules that depend on where the manifest sits are checked by `load_manifest(path, repo_root=...)` (and by `app-manifest validate-manifest` and `system/test_app_manifests.py`, which pass the real root): every referenced path must exist under the root, a reference inside the app's own `system/apps/<package>/` is rejected because that directory is already implicit, and a reference into any other `system/apps/<other>/` is rejected because an app-to-app dependency belongs in `pyproject.toml`, where it is already derivable. A manifest loaded from outside the `system/apps/<package>/app.toml` layout with no explicit root skips only those location checks; the value rules always apply. `app-manifest footprint` turns a manifest plus `system/supervisord.conf` into the scope file those passes read, and `app-manifest references --for-path` is the reverse lookup from an owned path back to its app.
